...
export const useCalendarLogic = (urlUid) => {
  ...
  const hasShownPrivateAlert = useRef(false); // 비공개 경고 중복 방지
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // (단, fetchUserData 로직에서 이미 리다이렉션이 발생하지만, 방어 코드 추가)
    ...
    // targetUser 상태가 확정된 후에만 기록을 불러오도록 의존성 추가
  ...
  }, [isOwnCalendar, outfitMap, currentUserId, navigate, profile]); // outfitMap이 변경될 때마다 갱신 필요
