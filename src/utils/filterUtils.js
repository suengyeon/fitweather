...
/**
 * 계절 코드를 정규화된 한글 레이블로 변환
 * @param {string} value - 계절 코드(영문or한글)
 * @returns {string} 정규화된 한글 계절명
 */
export const normalizeSeason = (value) => (value ? (seasonMap[value] || value) : "");

// 2. 스타일 코드↔별칭 매핑(한글/영문 모두 대응)
...
/**
 * 기록의 스타일 필드가 필터 키와 일치하는지 확인(단일 문자열or문자열 배열 모두 대응 가능)
 * @param {string | string[]} recordStyleField - 기록에 저장된 스타일 필드
 * @param {string} filterKey - 필터 드롭다운에서 선택된 영문 스타일 코드
